...
function iniTableForE() {
    const hr = document.getElementById('header-row3');
    const colNum = 100;

    // ヘッダー行の生成
    [...Array(colNum)].forEach((_, index) => {
        const th = document.createElement('th');
        th.innerText = `th${index + 1}番校`;
        hr.appendChild(th);
    });

    const tb = document.getElementById('table-body3');
    const rowNum = 15;

    // ボディ行の生成
    [...Array(rowNum)].forEach((_, rowIndex) => {
        const tr = document.createElement('tr');

        // セルの生成（colNum + 1 回回す）
        [...Array(colNum + 1)].forEach((_, colIndex) => {
            if (colIndex === 0) {
                const th = document.createElement('th');
                th.innerText = `th ${rowIndex + 1}番校`; // 元のコードの仕様通り
                tr.appendChild(th);
            } else {
                const td = document.createElement('td');
                td.innerText = `td${colIndex} 結果`;
                tr.appendChild(td);
            }
        });
        tb.appendChild(tr);
...
let tesarr = Array(9);
console.log(tesarr.length);
console.log(tesarr);
console.log(Object.keys(tesarr));

let tesarr2 = [...Array(9)];
console.log(tesarr2);
console.log(Object.keys(tesarr2));
